...
const Post = () => {
    ...
    const getPost = async () => {
      try {
        ...
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/post/get_posts?slug=${slug}`
        );
  ...
  useEffect(() => {
    const getRecentPosts = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/post/get_posts?limit=4`
        );
        if (res.status === 200) {
  ...
  const handleSummarizePost = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/ai/generate_post_summary`,
        { content: post.content }, {withCredentials : true}
      );
  ...
  if (!post || loading) return <LoaderPostPage />;
    ...
    <>
      <div className="default-padding pt-12 flex flex-col lg:flex-row items-start gap-6 relative">
        <div className="w-full md:flex-1 min-w-0">
          ...
          <div className="mt-2 md:mt-4 flex flex-wrap items-center gap-3 sm:gap-4">
            ...
            <button
              ...
              className="button-primary text-xs md:text-sm flex items-center gap-2 bg-gradient-to-r from-teal-600 to-teal-500 py-0.5 hover:from-primaryDark  duration-300"
            >
              <IoSparkles />
              Summerize Post
            </button>
        ...
        <Drawer
          ...
          title="Summary of the post"
        >
          <div className="mt-5">
            <h3 className="font-semibold mb-5 text-[17px]">
              {postSummary?.title}
            </h3>
            <MarkdownContent
              content={postSummary && sanitizeMarkdown(postSummary?.summary)}
            />
          </div>
        </Drawer>
      </div>
