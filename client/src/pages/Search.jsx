...
const Search = () => {
  ...
  const fetchPosts = async (startIndex = 0) => {
    try {
      ...
      const res = await axios.get(
        ...
        }/api/post/get_posts?startIndex=${startIndex}&${searchQuery}`
      );
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (
        scrollTop + clientHeight + 550 >= scrollHeight &&
        hasMore &&
        !loading
      ) {
        ...
        <div className="my-5">
          ...
          <div className="min-h-screen relative pb-20">
            {posts.length === 0 && loading ? (
              ...
              <h2 className="absolute top-[30%] left-1/2 -translate-x-1/2 text-2xl font-semibold italic text-center">
                {error}
              </h2>
            ) : (
